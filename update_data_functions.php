...
// This function saves an uploaded profile image securely in the database.
function save_profileimage($updated_value) {
    $file_name = $updated_value['name'];
    $file_path = "uploads/" . $file_name;
    move_uploaded_file($updated_value['tmp_name'], $file_path);
    $updated_value = $file_path;
...
function optimize_data_before_update($column_to_be_updated, $connection,
    ...
    switch ($column_to_be_updated) {
        ...
        case 'profile_image':
            save_profileimage($updated_value);
            return $updated_value;
            break;
...
/* Deletes the profile image so data is percerved more efficiently
   in the database.
*/
function deleteProfileImage($updated_value, $column_to_be_updated) {
    if ($column_to_be_updated == "profile_image") {
        unlink($updated_value);
    }
}
